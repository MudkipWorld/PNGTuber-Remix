...
func _physics_process(_delta: float) -> void:
	if dragging:
		...
		var size = %PointerHolder.size
		var pointer_offset = Vector2(20, 20)
		var snap_points = [
			Vector2(0, 0), Vector2(size.x / 2, 0), Vector2(size.x, 0),   
			Vector2(0, size.y / 2), Vector2(size.x / 2, size.y / 2), Vector2(size.x, size.y / 2), 
			Vector2(0, size.y), Vector2(size.x / 2, size.y), Vector2(size.x, size.y) 
		]
		...
		deform_x = clamp((closest.x) / size.x, 0.0, 1.0)
		deform_y = clamp((closest.y) / size.y, 0.0, 1.0)
		update_all_meshes_points()
