...
# Appearance
@export var warp_point_radius: float = 3.0
@export var glue_point_radius: float = 3.0
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed:
			# Start dragging the active warp
			dragging_warp = MeshEditor.active_warp
			drag_offset = mouse_pos
		else:
			dragging_warp = null

	elif event is InputEventMouseMotion and dragging_warp != null:
		deform_global_layer(dragging_warp, event.relative, mouse_pos)


func deform_global_layer(layer: DeformLayer, drag: Vector2, mouse_pos: Vector2):
	if layer == null:
		return
	...
	for grid_name in grids:
		...
		for i in range(points.size()):
			...
			match MeshEditor.brush_type:
				...
				1, 2:
					var offset = points[i] - mouse_pos
					offset = offset.rotated(drag.length() * 0.005 * influence * (1 if MeshEditor.brush_type == 1 else -1))
					points[i] += offset - (points[i] - mouse_pos)
				3, 4:
					var dir = (points[i] - mouse_pos).normalized()
					points[i] += dir * influence * (1 if MeshEditor.brush_type == 3 else -1)
				5:
...
# Check mouse over warp based on selected meshes
func is_mouse_over_warp(mouse_pos: Vector2, warp: DeformLayer) -> bool:
	...
	for sprite in Global.held_sprites:
		if sprite != null and is_instance_valid(sprite) and sprite.sprite_type == "Mesh":
			...
			if mesh != null and is_instance_valid(mesh) and warp.id in mesh.warps:
				meshes.append(mesh)
	...
	return Rect2(combined_min, combined_max - combined_min).has_point(mouse_pos)
